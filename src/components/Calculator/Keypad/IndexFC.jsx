import { KeypadWrapper } from "./styled";
import ButtonComponent from "./Button/indexFC";

const Keypad = () => {
	return (
		<KeypadWrapper>
			<ButtonComponent value={"1"} />
			<ButtonComponent value={"2"} />
			<ButtonComponent value={"3"} />
			<ButtonComponent value={"4"} />
			<ButtonComponent value={"5"} />
			<ButtonComponent value={"6"} />
			<ButtonComponent value={"7"} />
			<ButtonComponent value={"8"} />
			<ButtonComponent value={"9"} />
			<ButtonComponent value={"10"} />
			<ButtonComponent value={"11"} />
			<ButtonComponent value={"12"} />
			<ButtonComponent value={"13"} />
			<ButtonComponent value={"14"} />
			<ButtonComponent value={"15"} />
			<ButtonComponent value={"16"} />
			<ButtonComponent value={"17"} />
			<ButtonComponent value={"18"} />
			<ButtonComponent value={"19"} />
			<ButtonComponent value={"20"} />
		</KeypadWrapper>
	);
};

export default Keypad;